...
public class TWAgentWorkingMemory {
	...
	public void setObjects(TWAgentPercept[][] objects) {
	...
	public void updateMemory(Bag sensedObjects, IntBag objectXCoords, IntBag objectYCoords, Bag sensedAgents, IntBag agentXCoords, IntBag agentYCoords) {
		...
		//if the fuelPos is unknown, search it
		if(me.getStationPos() == null) {
			for(int i = 0; i < sensedObjects.size(); i++) {
				if(sensedObjects.get(i) instanceof TWFuelStation) {
					Int2D sp = new Int2D(((TWFuelStation) sensedObjects.get(i)).getX(), ((TWFuelStation) sensedObjects.get(i)).getY());
					me.setStationPos(sp);
				}
			}
		}
