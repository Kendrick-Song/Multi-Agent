...
public abstract class TWAgent extends TWEntity implements Steppable {
    ...
    // TODO: THE THREE METHODS YOU SHOULD EXTEND - COMMUNICATE, THINK, ACT
    ...
    protected final void pickUpTile(TWTile tile) {
        if (this.getEnvironment().canPickupTile(tile, this)) {
            if (carriedTiles.size() < 3) {
                ...
                System.out.println("WARN: Agent already carries 3 tiles.");
            }
        } else {
            System.out.println("WARN: The tile does not exist or the agent is not in the same position of the tile.");
        }
    }
    ...
    protected final void putTileInHole(TWHole hole) {
        if (this.getEnvironment().canPutDownTile(hole, this)) {
            ...
            System.out.println("INFO: Put tile...");
        } else {
            System.out.println("WARN: The put down action is invalid in current situation.");
        }
    }
    ...
    protected final void refuel() {
        ...
        if (this.getEnvironment().inFuelStation(this)) {
            ...
            System.out.println("INFO: Refuel.....");
        } else {
            System.out.println("WARN: Agent is not in the same position of fuel station.");
        }
    }
